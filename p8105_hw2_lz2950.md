...
``` r
#read csv file
Mr_trash_wheel <- read_excel("data/Trash Wheel Collection Data.xlsx", sheet = "Mr. Trash Wheel", range = cell_cols("A:N"), col_names = TRUE, trim_ws = TRUE, skip = 1) |>
...
``` r
prof_trash_wheel <- read_excel("data/Trash Wheel Collection Data.xlsx", sheet = "Professor Trash Wheel", range = cell_cols("A:M")) |>
  # use reasonable variable names
  janitor::clean_names() |>
  # omit rows that do not include dumpster-specific data
  filter(!is.na(dumpster)) |>
...
``` r
gwy_trash_wheel <- read_excel("data/Trash Wheel Collection Data.xlsx", sheet = "Professor Trash Wheel", range = cell_cols("A:K"), col_names = TRUE, trim_ws = TRUE, skip = 1) |>
  # use reasonable variable names
  janitor::clean_names() |>
  # omit rows that do not include dumpster-specific data
  filter(!is.na(dumpster)) |>
...
```
    ...
    ## Joining, by = c("dumpster", "month", "year", "date", "weight_tons",
    ## "volume_cubic_yards", "plastic_bottles", "polystyrene", "cigarette_butts",
    ## "glass_bottles", "grocery_bags", "homes_powered", "name")

The cleaned “Mr. Trash Wheel” data set has 547 observations of 15
variables and tells us about the trash collected by trash wheel from
May, 2014 to July, 2022. The cleaned “Professor Trash Wheel” data set
contains 94 observations of 14 variables from January, 2017 to July,
2022. The cleaned “Gwynnda Trash Wheel” data set contains 94
observations of 13 variables from January, 2017 to July, 2022.  
For the combined dataset, it contains 735 observations of 15 variables,
...
of cigarette butts collected by Gwynnda in July of 2021 is 1.56^{4}.
